...
// Builds the public URL a given object key resolves to.
...
// `explicitKey` overrides the generated random key — used by the one-off media
// migration script, which needs stable, re-runnable keys instead of a new UUID
// on every pass.
async function uploadToB2(file, folder, explicitKey) {
  if (!configured()) {
    throw new Error('Backblaze B2 is not configured — set B2_KEY_ID, B2_APP_KEY, B2_BUCKET, B2_ENDPOINT, B2_REGION.');
  }
  const ext = path.extname(file.originalname) || '';
  const key = explicitKey || `${folder}/${randomUUID()}${ext}`; // extension preserved so classify() keeps working
...
module.exports = { uploadToB2, configured, publicUrl };
