...
- `sessions` — connect-session-knex's table, explicit migration (not auto-created). **Its schema must match what the library itself would create** (`node_modules/connect-session-knex/lib/index.js`): `sid` string primary, `sess` json where supported else text, and **`expired` as `timestamp`**. The original migration declared `expired` as `bigInteger`, which SQLite accepted silently for the entire build; Postgres rejects the library's cleanup query (`expired < CAST($1 as timestamptz)` → error 42883 `bigint < timestamp with time zone`), the store throws from a timer, the process exits, no port binds, and the Render deploy fails its health check. Fixed in migration `20260907000009`.

  **General lesson:** SQLite locally and Postgres in production do not agree about types. Anything schema-shaped that works locally is not thereby verified — the first Postgres deploy is the real test.
